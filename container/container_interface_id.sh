...
IF_ID=$(${SUDO} docker exec ${CONTAINER_ID} cat /sys/class/net/eth0/iflink)
IF=$(${SUDO} ip a | awk -vpat=${IF_ID}: '$1 == pat {print $2}' | awk -F@ '{print $1}')

echo "Found container ${CYAN}${CONTAINER_ID}${NORMAL}/${CYAN}${CONTAINER_NAME}${NORMAL} matching '${CYAN}${SEARCH}${NORMAL}'.."
echo "  ..its veth pair on the host side is: ${CYAN}${IF}${NORMAL}"
